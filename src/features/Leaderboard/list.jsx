...
const Leaderboard = () => {
  ...
  return (
    <div className="w-screen rounded-3xl px-4 py-8">
      <h2 className="border border-white text-[32px] text-purple-300">
        Top Performers
      </h2>
      {selectedDataType === 'workout' && workoutCountData && (
        <div>
          <span className="border border-white text-[44px] text-purple-300">
            #8
          </span>
          <span className="border border-white text-sm font-medium text-[#B1B1B1]">
            of {workoutCountData.total} participants
          </span>
        </div>
      )}
      {selectedDataType === 'fitnessScore' && fitnessScoreData && (
        <div>
          <span className="border border-white text-[44px] text-purple-300">
            #8
          </span>
      ...
      {!fitnessScoreData && !workoutCountData ? (
        <>Loading.....</>
      ) : (
        (selectedDataType === 'workout'
