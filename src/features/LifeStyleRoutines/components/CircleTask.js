...
const CircleTask = ({ SelectedCircle, tasks, date, setShowCircleDetails }) => {
    ...
    return (
        <>
            {!showTaskDetail && <div className="w-full h-screen fixed top-0 left-0 z-50 p-2 bg-black text-white rounded-lg">
                ...
                <div className='p-2'>
                    {tasks.map((task, index) => (
                        <TaskItem key={index} task={task} index={index} onClick={() => setShowTaskDetail(true)}/>
                    ))}

                </div>
            </div>}
            {showTaskDetail && <TaskDetail SelectedCircle={SelectedCircle} tasks={tasks} setShowTaskDetail={setShowTaskDetail}/>}
        </>
    )
}
